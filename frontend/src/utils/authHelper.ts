...
export class AuthHelper {
  ...
  static logout() {
    // 清除所有本地存储的用户信息
    const keysToRemove = ['id', 'username', 'name', 'pwd', 'companyName', 'role', 'tel', 'img', 'token'];
   ...
   * 存储认证信息
   */
  static setAuth(user: any, token: string) {
    this.clearAuth() // 先清除旧的信息
    sessionStorage.setItem('token', token)
    sessionStorage.setItem('id', user.id.toString())
    sessionStorage.setItem('username', user.name) // 后端用户名字段是 name
    sessionStorage.setItem('role', user.role.toString())
    // 存储其他用户信息
    if (user.nickname) sessionStorage.setItem('nickname', user.nickname)
    if (user.tel) sessionStorage.setItem('tel', user.tel)
    if (user.email) sessionStorage.setItem('email', user.email)
    if (user.companyName) sessionStorage.setItem('companyName', user.companyName)
    if (user.sex) sessionStorage.setItem('sex', user.sex)
    if (user.imageUrl) sessionStorage.setItem('img', user.imageUrl)
    if (user.position) sessionStorage.setItem('position', user.position)
    
    console.log('AuthHelper.setAuth: 用户信息已存储', {
      id: user.id,
      username: user.name,
      role: user.role,
      完整信息: user
