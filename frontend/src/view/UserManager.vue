        ...
        <div>
          <div class="container">
            <div class="search-row">
              <div class="search-item">
                <h5>用户名称</h5>
                <el-input
                  style="width: 120px;margin-left:5px;margin-right:10px"
                  placeholder="请输入用户名称"
              ...
              <div class="search-item">
                ...
                <el-date-picker
                  ...
                  style="width: 280px; margin-left:5px; margin-right:10px"
                >
                </el-date-picker>
              </div>
              <div class="search-item" style="margin-top: 18px;">
          ...
          <el-dialog title="修改用户" v-model="dialogVisible" width="40%"
            ...
            <el-form ref="userRef" :model="user" label-width="80px"
              ...
              <el-form-item>
                <el-button type="primary" @click="updateUser" style="margin-left: 180px">提交</el-button>
                <el-button @click="back">返回</el-button>
              </el-form-item>
            </el-form>
          </el-dialog>
...
import { searchUsers, searchUsersByDepartment, searchUsersByCompany, updateUserState, deleteUsers, deleteUsersFormData, addUser as apiAddUser, updateUser as apiUpdateUser } from '@/api/user';
...
function openForm(row: any) {
  ...
  // 转换性别数据：从数据库的 man/woman 转换为表单的 1/0
  if (user.value.sex === 'man') {
    user.value.sex = '1';
  } else if (user.value.sex === 'woman') {
    user.value.sex = '0';
  } else {
    user.value.sex = '1'; // 默认为男
  }
...
const addUser = () => {
  ...
  userRef.value.validate(async (valid) => {
    if (valid) {
      ...
      if (user.value.sex === '1') {
        user.value.sex = 'man';
      } else if (user.value.sex === '0') {
        user.value.sex = 'woman';
      }
      
      let res = await apiAddUser(user.value);
      if (res.isOK) {
        ElMessage({
          message: res.msg,
          type: 'success'
...
const updateUser = () => {
  ...
  userRef.value.validate(async (valid) => {
    if (valid) {
      ...
      // 设置部门ID和部门名称
      if (value.value) {
        const selectedDepartment = ValueDFS(dataSource.value[0], value.value)[0];
        if (selectedDepartment) {
          user.value.secId = String(selectedDepartment.departmentId);
          user.value.section = selectedDepartment.label;
          user.value.companyName = selectedDepartment.companyName;
        }
      }

      let res = await apiUpdateUser(user.value);
      if (res.isOK) {
        ElMessage({
          message: res.msg,
          type: 'success'
...
async function search() {
  ...
  // 使用JSON格式发送搜索参数，与后端API期望的格式一致
  const searchData: any = {};
  if (name) searchData.name = name;
  if (nickname) searchData.nickname = nickname;
  if (tel) searchData.tel = tel;
  ...
  try {
    // 使用service调用，发送JSON格式数据
    const response = await service.post('/user/search', searchData);

    if (response && (response.isOK || response.isOk)) {
