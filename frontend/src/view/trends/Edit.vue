  ...
  formRef.value.validate((valid: any) => {
    ...
    if (valid) {
      ...
      const updateData = {
        ...
        isAdmin: sessionStorage.getItem('name') === 'admin' || currentUserRole === '1' || currentUserRole === '2', // 明确标识是否为管理员
      ...
      console.log('是否为管理员:', sessionStorage.getItem('name') === 'admin' || currentUserRole === '1' || currentUserRole === '2');
      console.log('========================');
      
      // 根据用户角色选择使用不同的API
      const isAdmin = sessionStorage.getItem('name') === 'admin' || currentUserRole === '1' || currentUserRole === '2';
...
onBeforeMount(() => {
  ...
  // 修复权限检查逻辑：超级管理员(用户名admin)或系统管理员(角色1)或企业管理员(角色2)都可以编辑
  const userRole = sessionStorage.getItem('role');
  const userName = sessionStorage.getItem('name');
  if (sessionStorage.getItem("canEdit") === 'true' || userName === 'admin' || userRole === '1' || userRole === '2') {
    disabledForm.value = false
    console.log('用户有编辑权限，启用表单编辑');
  } else {
    console.log('用户无编辑权限，表单保持禁用状态');
  }
