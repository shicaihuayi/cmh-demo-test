  ...
  <div class="tenant-manager">
    ...
    <el-table
      ...
      >
        ...
        <el-table-column prop="mail" label="邮箱" width="161" header-align="center" align="center" show-overflow-tooltip />
        
        <el-table-column prop="desc" label="描述" width="162" header-align="center" align="center" show-overflow-tooltip />
        ...
        <el-table-column label="操作" width="274" fixed="right" header-align="center" align="center">
          <template #default="{ row }">
...
const loadTenants = async () => {
  try {
    ...
    // 兼容后端不一致的属性名
    if (response && (response.isOK === true || response.isOk === true)) {
...
const handleSearch = async () => {
  ...
  if (hasSearchCondition) {
    try {
      ...
      // 兼容后端不一致的属性名
      if (response.isOK === true || response.isOk === true) {
        allTenants.value = response.comps || []
...
const handleSubmit = async () => {
  ...
  try {
    ...
    if (dialogMode.value === 'add') {
      ...
      const originalName = originalTenant.value?.name || tenantData.name
      console.log('原始公司名称:', originalName)
      response = await updateTenant(tenantData, originalName)
    ...
    // 兼容后端不一致的属性名：有些接口返回isOK，有些返回isOk
    const isSuccess = response && (response.isOk === true || response.isOK === true)
