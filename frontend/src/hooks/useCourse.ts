...
export interface Course {
  ...
  department?: string;     // 创建者部门
  creator_role?: string;   // 创建者角色
}

export default function () {
  const task = useTaskStore();
  const dashboardStore = useDashboardStore(); // 新增: 引入仪表盘状态存储
  ...
  async function search() {
    ...
    try {
      ...
      console.log('开始搜索课程，条件:', searchCourse.value);
  ...
  return {
    ...
    refreshDashboard, // 导出刷新仪表盘功能，以便在其他地方使用
  };
}
