...
export default function () {
            ...
            if (res.isOk) {
                ...
                try {
                    ...
                    if (userData.imageUrl && userData.imageUrl.trim()) {
                        ...
                        // 同步更新sessionStorage和userStore
                        sessionStorage.setItem('img', userData.imageUrl);
                        userStore.updateHeadImg(userInfo.imageUrl);
                    } else {
                        // 使用项目中已存在的默认头像
                        userInfo.imageUrl = '/src/img/头像.jpg';
                        // 清除sessionStorage中的头像信息
                ...
                user.imgUrl = userInfo.imageUrl; // 同步头像URL
                
                console.log('用户数据加载完成，头像URL:', userInfo.imageUrl);
    ...
    function modifyImage() {
        ...
        service.post('/user/updateImage', fd).then((res:any) => {
            if (res.isOk) {
                ElMessage.success(res.msg);
                ...
                // 同步更新sessionStorage和userStore，确保导航栏头像实时更新
                const fullImageUrl = user.imgUrl.startsWith('http') 
                    ? user.imgUrl 
                    : `http://localhost:8080${user.imgUrl}`;
                
                // 更新sessionStorage
                sessionStorage.setItem('img', user.imgUrl);
                
                // 更新userStore，立即同步到导航栏
                userStore.updateHeadImg(fullImageUrl);
                
                console.log('头像更新成功，已同步到导航栏');
            } else {
                ElMessage.error(res.msg);
            }
        });
