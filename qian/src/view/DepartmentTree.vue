<template>
  <!-- 外部添加部门按钮 -->
  <el-button type="primary" @click="openAddForm()" style="margin-left: 12px; margin-bottom: 4px">添加</el-button>
  <!-- 外部添加部门表单对话框 -->
  <el-dialog v-model="addFormDialog" title="添加部门" width="500" @close="closeAdd">
    <el-form :model="departmentForm" :rules="departmentRules" ref="departmentFormRef">
      ...
      <el-form-item label="显示排序" prop="superior">
        <el-space>
...
function add() {
  ...
  if (value.value <= 0) {
    if (dataSource.value[0].children) {
      for (let company of dataSource.value[0].children) {
        if (company.value == value.value) {
          departmentfd.append('superior', company.departmentId as unknown as string);
          ...
          departmentfd.append('departmentLevel', 1 as unknown as string);
          departmentfd.append('companyName', company.companyName);
    ...
    let parent = IdDFS(dataSource.value[0], value.value)[0]
    let level = parent.departmentLevel + 1
    departmentfd.append('superior', value.value as unknown as string);
    departmentfd.append('departmentName', departmentForm.departmentName);
    departmentfd.append('order', departmentForm.order as unknown as string);
    departmentfd.append('director', departmentForm.director);
    departmentfd.append('tel', departmentForm.tel);
    ...
    departmentfd.append('companyName', parent.companyName);
    axios.post('http://localhost:8080/department/add', departmentfd).then((result) => {
      if (result.data.isOk) {
        dataSource.value = [{
          value: 0,
