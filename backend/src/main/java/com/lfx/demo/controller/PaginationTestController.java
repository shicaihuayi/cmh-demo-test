...
public class PaginationTestController {
    ...
    public AjaxResult testBasicPagination(
        ...
        result.put("输入参数", new HashMap<>() {{ put("pageNum", pageNum); put("pageSize", pageSize); }});
    ...
    public AjaxResult testMultipleParameterNames(
        ...
        Map<String, Object> result = new HashMap<>();
        result.put("输入参数", new HashMap<>() {{
                put("pageNum", pageNum); put("page", page); put("current", current);
                put("pageSize", pageSize); put("size", size); put("limit", limit);
        }});
